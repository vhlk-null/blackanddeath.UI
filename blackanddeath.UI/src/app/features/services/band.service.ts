...
export class BandService {
  ...
  getAllPaginated(params: { pageIndex: number; pageSize: number; sortBy?: string; sortDir?: string; name?: string; genreId?: string; genreName?: string[]; countryId?: string; countryName?: string[]; status?: string[]; yearFrom?: string; yearTo?: string }) {
    return this.http.get<BandsResponse>(BandEndpoints.GET_ALL, params).pipe(
      map(response => response.bands)
    );
  }
